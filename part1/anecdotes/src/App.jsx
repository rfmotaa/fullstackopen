...
const App = () => {
  ...
  function newVote() {
    ...
    setVotes(newVotes)
    findMaxVoted(newVotes)
  }

  function findMaxVoted() {
    let actual = 0;
    for (let i = 0; i < anecdotes.length; i++) {
      if (votes[i] > votes[actual]) {
        actual = i
      }
    }
    setMaxVoted(actual)
  ...
  return (
    <div>
      ...
      <button onClick={() => {newVote()}}>vote</button> <button onClick={() => {setSelected(generateRandom())}}>next anecdote</button>
